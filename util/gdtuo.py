...
class Optimizable:
    ...
    def begin_w(self):
        ...
        for name, param in self.parameters.items():
            ...
            if not (name.endswith("a") or name.endswith("a_n") or name.endswith("a_p") ):
                self.all_params_with_gradients.append(param)
...
class SGD_Delayed_Updates(Optimizable):
    ...
    def step_a(self, params,modules_to_quantize,excepts):# step on the a parameters
        ...
        for name, param in params.items():
            if (convert_name(name) in modules_to_quantize.keys()) and (name.endswith("quan_w_fn.a") or name.endswith("quan_w_fn.a_n") or name.endswith("quan_w_fn.a_p")) and not(convert_name(name) in excepts.keys()):
    ...
    def step_w(self, params,modules_to_quantize,excepts):# step on the rest of the parametrs
        ...
        for name, param in params.items():
            if (convert_name(name) in modules_to_quantize.keys()) and name.endswith("weight")and not(convert_name(name) in excepts.keys()):# if this is a weight of a quantized    
                ...
                if not((convert_name(name) in modules_to_quantize.keys()) and (name.endswith("quan_w_fn.a") or name.endswith("quan_w_fn.a_n") or name.endswith("quan_w_fn.a_p")) and not(convert_name(name) in excepts.keys())):
...
class SGD_Delayed_Updates_DualPWL(Optimizable):
    '''
    A hyperoptimizable SGD.
    '''
